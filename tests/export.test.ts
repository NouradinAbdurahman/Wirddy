...
import { buildMemberCardHtml, buildWeeklyTableSectionHtml, buildWeeklyCardsSectionHtml, buildStandaloneWeekExportHtml } from '../lib/export/render-week';
...
describe('Export System: Binary Signature Validation', () => {
  ...
  it('rejects an invalid or corrupted PNG binary', async () => {
    const fakeData = new Uint8Array([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77]);
...
describe('Export System: Table View Rendering & Theme Awareness', () => {
  it('renders premium table HTML with table columns and member data', () => {
    const scheduleInput = {
      ...
      members: [
        { id: '1', name: 'طارق', knowledgeType: 'entire' as const, startJuz: 1, endJuz: 30, weeklyAmount: 10 },
        { id: '2', name: 'زينب', knowledgeType: 'entire' as const, startJuz: 1, endJuz: 30, weeklyAmount: 10 },
        { id: '3', name: 'يوسف', knowledgeType: 'entire' as const, startJuz: 1, endJuz: 30, weeklyAmount: 10 },
      ],
    };

    const schedule = generateQuranSchedule(scheduleInput);
    const exportWeek = normalizeWeekSchedule(schedule.weeks[0], 2, schedule.groupName, 'ar', 'dark', 'table');
  ...
  it('renders standalone export with Light theme and correct logo when requested', () => {
    const scheduleInput = {
      ...
      members: [
        { id: '1', name: 'Tariq', knowledgeType: 'entire' as const, startJuz: 1, endJuz: 30, weeklyAmount: 15 },
        { id: '2', name: 'Zainab', knowledgeType: 'entire' as const, startJuz: 1, endJuz: 30, weeklyAmount: 15 },
      ],
    };

    const schedule = generateQuranSchedule(scheduleInput);
    const exportWeek = normalizeWeekSchedule(schedule.weeks[0], 1, schedule.groupName, 'en', 'light', 'table');
    ...
    const standaloneHtml = buildStandaloneWeekExportHtml(exportWeek, assets, 'light', 'table');
...
describe('Export System: Dynamic Content-Aware A4 PDF Pagination', () => {
  it('packs a 5-week plan with 3 members into exactly 3 PDF pages (2 + 2 + 1)', () => {
    ...
    const schedule = generateQuranSchedule(scheduleInput);
    const exportSchedule = normalizeScheduleToExport(schedule, 'ar', 'dark', 'cards');
