...
import { normalizeApiAyah, normalizeApiJuzData, normalizeApiSurah } from '../lib/quran/normalize';
...
describe('AlQuran Cloud API Integration & Quran Data Model', () => {
  it('loads all 30 verified Juz boundaries', () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      expect(boundaries[i].start.globalAyahNumber).toBeLessThanOrEqual(boundaries[i].end.globalAyahNumber);
    }
  });
  ...
  it('validates sequential continuity of all 30 Juz with zero gaps and zero overlaps', () => {
    ...
    for (let i = 1; i < 30; i++) {
      expect(boundaries[i].start.globalAyahNumber).toBe(boundaries[i - 1].end.globalAyahNumber + 1);
    }
  });
