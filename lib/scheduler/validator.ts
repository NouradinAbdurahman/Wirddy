import { GeneratedSchedule, MemberConfig, ScheduleInput, SchedulerValidationError, SchedulerValidationResult } from './types';

export function validateScheduleInput(input: ScheduleInput): SchedulerValidationResult {
...
export function validateGeneratedSchedule(schedule: GeneratedSchedule, input: ScheduleInput): SchedulerValidationResult {
  const errors: SchedulerValidationError[] = [];

  if (!schedule || !schedule.weeks || schedule.weeks.length !== input.group.weeksCount) {
    errors.push({
  ...
  schedule.weeks.forEach((week) => {
    ...
    week.assignments.forEach((assignment) => {
      ...
      if (assignment.startJuz < member.startJuz || assignment.endJuz > member.endJuz) {
        errors.push({
      ...
      if (!assignment.startAyah || !assignment.endAyah || !assignment.startAyah.surahNumber || !assignment.endAyah.surahNumber) {
        errors.push({
