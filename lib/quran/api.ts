...
async function fetchApi<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Quran API HTTP ${response.status}: ${response.statusText}`);
    }
 ...
 * GET /surah/{number}
 */
export async function fetchSurah(surahNumber: number): Promise<ApiSurahReference & { ayahs: ApiAyah[] }> {
  if (surahNumber < 1 || surahNumber > 114) {
    throw new Error(`Invalid Surah number: ${surahNumber}. Must be between 1 and 114.`);
  }
  return fetchApi<ApiSurahReference & { ayahs: ApiAyah[] }>(`/surah/${surahNumber}`);
}
 ...
 */
export async function fetchJuz(juzNumber: number, edition?: string): Promise<ApiJuzData> {
  if (juzNumber < 1 || juzNumber > 30) {
    throw new Error(`Invalid Juz number: ${juzNumber}. Must be between 1 and 30.`);
  }
  const endpoint = edition ? `/juz/${juzNumber}/${edition}` : `/juz/${juzNumber}`;
 ...
 * GET /ayah/{number}
 */
export async function fetchAyah(globalAyahNumber: number, edition?: string): Promise<ApiAyah> {
  ...
  const endpoint = edition ? `/ayah/${globalAyahNumber}/${edition}` : `/ayah/${globalAyahNumber}`;
