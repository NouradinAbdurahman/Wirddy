 ...
 */
export function validateJuzBoundaries(
  ...
  if (boundaries.length !== 30) {
    errors.push(
      `Expected 30 Juz boundaries, but received ${boundaries.length}.`
    )
  ...
  for (let i = 0; i < boundaries.length; i++) {
    ...
    if (boundary.juzNumber !== expectedJuz) {
      errors.push(
        `Boundary at index ${i} has Juz number ${boundary.juzNumber}, expected ${expectedJuz}.`
      )
    }
    ...
    if (boundary.start.globalAyahNumber > boundary.end.globalAyahNumber) {
      errors.push(
        `Juz ${expectedJuz} start global Ayah (${boundary.start.globalAyahNumber}) is greater than end global Ayah (${boundary.end.globalAyahNumber}).`
      )
    }
    ...
    if (i > 0) {
      ...
      if (prevBoundary.end && boundary.start) {
        if (
          boundary.start.globalAyahNumber !==
          prevBoundary.end.globalAyahNumber + 1
        ) {
          errors.push(
  ...
  if (boundaries[0]?.start?.globalAyahNumber !== 1) {
    errors.push(
      `Juz 1 must start at global Ayah 1 (received ${boundaries[0]?.start?.globalAyahNumber}).`
    )
  }

  if (boundaries[29]?.end?.globalAyahNumber !== 6236) {
    errors.push(
      `Juz 30 must end at global Ayah 6236 (received ${boundaries[29]?.end?.globalAyahNumber}).`
    )
  }
...
export async function fetchAndDeriveAllJuzBoundaries(
  ...
  if (!validation.isValid) {
    throw new Error(
      `Juz boundary validation failed:\n- ${validation.errors.join("\n- ")}`
    )
  }
