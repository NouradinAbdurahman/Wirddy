...
import { ExportProgressCallback, ExportRenderOptions, ExportSchedule } from './types';
...
export async function exportAllWeeksAsZip(
  ...
  for (let i = 0; i < totalWeeks; i++) {
    ...
    if (!validation.isValid) {
      throw new Error(`Week ${week.weekNumber} PNG validation failed: ${validation.error}`);
    }
  ...
  const zipFilename = getZipExportFilename(schedule.groupName, schedule.language);
