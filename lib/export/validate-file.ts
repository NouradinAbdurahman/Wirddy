...
export async function validatePngBlob(blob: Blob): Promise<ValidationResult> {
  ...
  if (blob.type && !blob.type.includes("png")) {
    return {
      isValid: false,
      error: `Invalid MIME type for PNG: "${blob.type}".`,
    }
  }
  ...
  try {
    ...
    if (bytes.length < 8) {
      return {
        isValid: false,
        error: "File too small to contain a valid PNG header.",
      }
    }
    ...
    for (let i = 0; i < 8; i++) {
      if (bytes[i] !== pngSignature[i]) {
        return {
          isValid: false,
          error: "PNG magic bytes signature check failed.",
        }
      }
    }
    ...
    return {
      isValid: false,
      error: `Failed to inspect PNG binary: ${String(err)}`,
    }
  }
}
...
export async function validatePdfBlob(blob: Blob): Promise<ValidationResult> {
  ...
  if (blob.type && !blob.type.includes("pdf")) {
    return {
      isValid: false,
      error: `Invalid MIME type for PDF: "${blob.type}".`,
    }
  }
  ...
  try {
    ...
    if (bytes.length < 5) {
      return {
        isValid: false,
        error: "File too small to contain a valid PDF header.",
      }
    }
    ...
    for (let i = 0; i < 5; i++) {
      if (bytes[i] !== pdfSignature[i]) {
        return {
          isValid: false,
          error: "PDF signature check failed (missing %PDF- header).",
        }
      }
    }
    ...
    return {
      isValid: false,
      error: `Failed to inspect PDF binary: ${String(err)}`,
    }
  }
}
...
export async function validateZipBlob(blob: Blob): Promise<ValidationResult> {
  ...
  try {
    ...
    if (bytes.length < 4) {
      return {
        isValid: false,
        error: "File too small to contain a valid ZIP header.",
      }
    }
    ...
    for (let i = 0; i < 4; i++) {
      if (bytes[i] !== zipSignature[i]) {
        return {
          isValid: false,
          error: "ZIP signature check failed (missing PK header).",
        }
      }
    }
    ...
    return {
      isValid: false,
      error: `Failed to inspect ZIP binary: ${String(err)}`,
    }
  }
}
