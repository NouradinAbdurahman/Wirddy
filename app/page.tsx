...
export default function HomePage() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed.members) && parsed.members.length > 0) setMembers(parsed.members);
        if (parsed.schedule) {
  ...
  // Calculate live sum
  const currentTotal = members.reduce((sum, m) => sum + (m.weeklyAmount || 0), 0);

  const inputPayload: ScheduleInput = {
    group: { name: groupName || (language === 'ar' ? 'مجموعتي' : 'My Group'), weeksCount },
    members,
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-background text-foreground selection:bg-primary/20 selection:text-primary">
      {/* App Header */}
      <Header
      ...
      {/* Main Content Area */}
      <main className="flex-1 w-full container mx-auto max-w-6xl px-4 sm:px-6 py-6 md:py-10">
        <AnimatePresence mode="wait">
          ...
          {step === 'landing' && (
            ...
            >
              <Hero onCreateGroup={startNewGroup} onHowItWorks={scrollToHowItWorks} />
              <HowItWorks />
              <ExampleSchedule />
              <Features />
              ...
              <section className="py-12 text-center">
                <Card className="border border-border/60 bg-card/60 backdrop-blur-md rounded-3xl p-8 sm:p-12 shadow-lg max-w-3xl mx-auto space-y-6">
                  <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-3xl overflow-hidden flex items-center justify-center p-2.5 sm:p-3 bg-muted/50 dark:bg-muted/20 border border-border/60 shadow-md mx-auto">
                    <img
                      src="/logo-black.png"
                      alt="Wirddy"
                  ...
                  <Button
                    ...
                    className="h-12 px-8 text-base font-semibold rounded-xl shadow-md gap-2 mx-auto"
                  >
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <Button
                  ...
                  className="rounded-xl px-3 text-xs font-semibold gap-1.5 text-muted-foreground hover:text-foreground"
                >
              ...
              <div className="sticky top-16 z-20 -mx-2 px-2 py-2 bg-background/85 backdrop-blur-md transition-all">
